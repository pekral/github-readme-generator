...
[`docs/examples/`](docs/examples/README.md) puts three README pairs side by side —
...
Read them for what they are: on those repositories the baseline invented nothing,
so the pairs show ordering, evidence-backed badges and referenced-rather-than-copied
output — and one case where the skill did worse than the baseline. The index says
so plainly.
